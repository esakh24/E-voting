...
class AccountCheckMiddleWare(MiddlewareMixin):
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        if user.is_authenticated:
            ...
            if request.path == reverse('account_login') or request.path == reverse('forget_password') or modulename == 'django.contrib.auth.views' or request.path == reverse('send_mail') or request.path == reverse('reset_password'):
